 ...
 * ─────────────────────────────────────
 * Partie 2 du tunnel de réservation.
 * Sélection date, heure début, heure fin + résumé calculé.
 */
...
function estimatedPrice(startTime: string, endTime: string, pricePerHour: number): number {
  const [sh, sm] = startTime.split(":").map(Number);
  const [eh, em] = endTime.split(":").map(Number);
  const total = (eh * 60 + em) - (sh * 60 + sm);
  if (total <= 0) return 0;
  return Math.round((total / 60) * pricePerHour);
}

export default function ReservationForm() {
  const { lotId, name, spotId, pricePerHour } = useLocalSearchParams<{
    lotId:        string;
    name:         string;
    spotId:       string;
    pricePerHour: string;
  ...
  async function handleCheck() {
    ...
    setChecking(true);
    try {
      const url = `${BACKEND_URL}/reservations/check?lot_id=${lotId}&spot_id=${spotId}&date=${selectedDay}&start=${startTime}&end=${endTime}`;
      ...
      if (data.available) {
        ...
        Alert.alert("Créneau indisponible", data.message ?? "Cet emplacement est déjà réservé sur ce créneau.");
      }
    } catch {
      Alert.alert("Erreur réseau", "Impossible de vérifier la disponibilité.");
    } finally {
      setChecking(false);
    }
  }
  ...
  return (
    <ScrollView style={s.screen} contentContainerStyle={s.content} keyboardShouldPersistTaps="handled">
      ...
      <View style={s.header}>
        ...
        <Text style={s.headerSub}>{name} · Place N°{spotId}</Text>
      </View>
      ...
      <SectionCard icon="calendar-outline" title="Date">
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={s.dayScroll}>
          {DAYS.map((d) => (
            ...
            >
              <Text style={[s.dayChipText, selectedDay === d.value && s.dayChipTextActive]}>
                {d.label}
              </Text>
            </TouchableOpacity>
          ))}
      ...
      <SectionCard icon="time-outline" title="Horaires">
        <View style={s.timeRow}>
          ...
          <View style={s.timeBlock}>
            ...
            <TouchableOpacity style={s.timePicker} onPress={() => { setShowStartPick(!showStartPick); setShowEndPick(false); }}>
            ...
            {showStartPick && (
              <ScrollView style={s.timeDropdown} nestedScrollEnabled>
                {TIME_SLOTS.map((t) => (
                  <TouchableOpacity
                    key={t}
                    style={[s.timeOption, t === startTime && s.timeOptionActive]}
                    onPress={() => { setStartTime(t); setShowStartPick(false); }}
                  >
                    <Text style={[s.timeOptionText, t === startTime && s.timeOptionTextActive]}>{t}</Text>
                  </TouchableOpacity>
                ))}
              </ScrollView>
            )}
          ...
          <Ionicons name="arrow-forward" size={18} color="#94a3b8" style={s.timeArrow} />

          {/* End time */}
          <View style={s.timeBlock}>
            <Text style={s.timeLabel}>Fin</Text>
            <TouchableOpacity style={s.timePicker} onPress={() => { setShowEndPick(!showEndPick); setShowStartPick(false); }}>
              ...
              <ScrollView style={s.timeDropdown} nestedScrollEnabled>
                {TIME_SLOTS.map((t) => (
                  <TouchableOpacity
                    ...
                    onPress={() => { setEndTime(t); setShowEndPick(false); }}
                  >
                    <Text style={[s.timeOptionText, t === endTime && s.timeOptionTextActive]}>{t}</Text>
                  </TouchableOpacity>
                ))}
              </ScrollView>
            )}
      ...
      {/* ── Section : résumé ─────────────────────────────────────────────── */}
      <SectionCard icon="receipt-outline" title="Résumé">
        <View style={s.summaryGrid}>
          <SummaryRow label="Durée totale"  value={duration} />
          <SummaryRow label="Prix estimé"   value={isValid ? `${estPrice} DA` : "—"} highlight />
          <SummaryRow label="Tarif"         value={`${price} DA / heure`} />
          <SummaryRow label="Date"          value={selectedDay} />
          <SummaryRow label="Créneau"       value={isValid ? `${startTime} → ${endTime}` : "—"} />
        </View>
      </SectionCard>

      {/* ── CTA ─────────────────────────────────────────────────────────────── */}
      <TouchableOpacity
        style={[s.btn, (!isValid || checking) && s.btnOff]}
        onPress={handleCheck}
        disabled={!isValid || checking}
      >
        {checking
          ? <ActivityIndicator color="#fff" />
          : <Text style={s.btnText}>Vérifier disponibilité  →</Text>
        }
      </TouchableOpacity>
    </ScrollView>
  );
...
function SummaryRow({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <View style={s.summaryRow}>
      ...
      <Text style={[s.summaryValue, highlight && s.summaryHighlight]}>{value}</Text>
    </View>
  );
}
...
const s = StyleSheet.create({
  ...
  timeBlock:{ flex: 1 },
  timeLabel:{ fontSize: 12, color: "#94a3b8", fontWeight: "600", marginBottom: 6 },
  timePicker: {
    flexDirection: "row", alignItems: "center", justifyContent: "space-between",
    backgroundColor: "#f7f9fc", borderRadius: 10, borderWidth: 1.5,
    borderColor: "#e2e8f0", paddingHorizontal: 12, height: 44,
  },
